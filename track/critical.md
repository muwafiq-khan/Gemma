...
This file records major changes, architecture decisions, and root-cause analyses.
Agent will NOT write here without user permission.
...
- **Root cause:** `chat_fn` was re-sending the **full** profile interview prompt with every user message, causing Gemma to restart the interview loop instead of progressing toward JSON completion
...
- **Helper text:** Added instruction above the chat
