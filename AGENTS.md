...
### RAG-Powered Personalization (Future)
- Profile user: genre preferences, past favorite movies/games, DSA strengths/weaknesses
- Fetch from Wikipedia + TV Tropes based on user's taste
- Generate a custom skeleton inspired by Shakespeare (if they love drama) or horror movie structure (if they love horror) or a specific game's narrative
- For demo: skip RAG, use a pre-defined genre + let Gemma 4 generate skeleton from guidance
